...
data = pd.read_csv("../covid-19-excess-deaths-tracker/output-data/excess-deaths/spain_excess_deaths.csv")

covid_deaths = data[data['region'] == 'Spain']

dates = covid_deaths['start_date'].to_numpy()
aggregate = data.groupby('start_date').sum()
excess_deaths = aggregate['excess_deaths'].to_numpy()
#excess_deaths_h = covid_deaths['excess_deaths.lwr'].to_numpy()
#excess_deaths_l = covid_deaths['excess_deaths.upr'].to_numpy()

covid_deaths_data = aggregate['covid_deaths'].to_numpy()
delta = 0

cum_sum = np.cumsum(excess_deaths)
cum_sum_covid = np.cumsum(covid_deaths_data)

cum_std = 0#np.asarray([1.96*np.linalg.norm(delta[:i]) for i in range(len(delta))])

population = 46.94e6
prefactor = 1/population*1e5
...
ax.text(datetime.date(2020, 1, 5), 0.89*60, r'(b)')
ax2 = ax.twinx()
...
plt.tight_layout()
plt.savefig('Spain_Covid_excess.png', dpi = 480)
plt.show()