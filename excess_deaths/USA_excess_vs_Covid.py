...
covid_deaths = data[data['region_code'] == 'USA']
...
US_population = 328.2e6
prefactor = 1/US_population*1e5

#%%
fig, ax = plt.subplots(figsize = fig_size)

ax.errorbar(100, 100, color = 'k', label = r'$\delta D_{\mathrm{c}}$')
ax.errorbar(100, 100, color = '#223A27', ecolor='#223A27', elinewidth=1, \
            capsize=2, label = r'$\delta D_{\mathrm{e}}$')

ax.errorbar(100, 100, color = 'DarkRed', ecolor='k', elinewidth=1, \
            capsize=2, label = r'$D_{\mathrm{c}}$')

ax.errorbar(100, 100, color = '#C81C1A', ecolor='#C81C1A', \
            elinewidth=1, capsize=2, label = r'$D_{\mathrm{e}}$')

ax.legend(loc = 3, ncol = 4, fontsize = 8, frameon = False)
ax.grid()
plt.savefig("legend2.png", dpi = 480)
plt.close()
...
fig, ax = plt.subplots(figsize = fig_size)
ax.set_title(r'USA')
ax.text(datetime.date(2020, 1, 5), 0.89*10, r'(a)')
ax2 = ax.twinx()
...
plt.tight_layout()
plt.savefig('USA_Covid_excess.png', dpi = 480)
plt.show()