...
data = pd.read_csv("../covid-19-excess-deaths-tracker/output-data/excess-deaths/sweden_excess_deaths.csv")

covid_deaths = data[data['region'] == 'Sweden']

dates = covid_deaths['start_date'].to_numpy()
aggregate = data.groupby('start_date').sum()
excess_deaths = aggregate['excess_deaths.fit'].to_numpy()
excess_deaths_h = aggregate['excess_deaths.lwr'].to_numpy()
excess_deaths_l = aggregate['excess_deaths.upr'].to_numpy()

covid_deaths_data = aggregate['covid_deaths'].to_numpy()
delta = excess_deaths_h-excess_deaths_l

cum_sum = np.cumsum(excess_deaths)
cum_sum_covid = np.cumsum(covid_deaths_data)

cum_std = np.asarray([1.96*np.linalg.norm(delta[:i]) for i in range(len(delta))])
...
ax.text(datetime.date(2020, 1, 5), 0.89*10, r'(c)')
ax2 = ax.twinx()
...
plt.tight_layout()
plt.savefig('Sweden_Covid_excess.png', dpi = 480)
plt.show()