...
data = pd.read_csv("../covid-19-excess-deaths-tracker/output-data/excess-deaths/united_states_excess_deaths.csv")

covid_deaths = data[data['region_code'] == 'NYC']
...
cum_std = np.asarray([1.96*np.linalg.norm(delta[:i])*(i+1)/i if i > 0 \
                      else 1.96*np.linalg.norm(delta[:i]) for i in range(len(delta))])

NYC_population = 8.399e6
prefactor = 1/NYC_population*1e5
...
ax.set_title(r'New York City')
ax.text(datetime.date(2020, 1, 5), 0.89*100, r'(b)')
ax2 = ax.twinx()
...
ax.errorbar(dates, prefactor*covid_deaths_data, color = 'k', label = r'$\delta D_{\mathrm{c}}$')

ax.errorbar(dates, prefactor*excess_deaths, yerr = prefactor*delta, \
            color = '#223A27', ecolor='#223A27', elinewidth=1, capsize=2, \
            label = r'$\delta D_{\mathrm{e}}$')

ax2.errorbar(dates, prefactor*cum_sum_covid, color = 'DarkRed', ecolor='k', \
             elinewidth=1, capsize=2, label = r'$D_{\mathrm{c}}$')
...
plt.tight_layout()
plt.savefig('NYC_Covid_excess.png', dpi = 480)
plt.show()